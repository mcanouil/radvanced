...
# Fondamentaux | "<i class="fab fa-r-project"></i> base" {.flexbox .vcenter}


## Fondamentaux | Affectation

```{r, eval = FALSE}
x <- c(1, 2, 3)
```

> Crée un objet nommé "x", contenant les valeurs 1, 2 et 3

Ou plus précisément :
* Crée un objet de type vecteur `c(1, 2, 3)`.
* Associe cet objet à un nom `x`.
    `x` est donc une référence à l'objet.
