...
.footnote.font150[
&ast; Le *workspace* est automatiquement sauvegardé/chargé via un fichier nommé `.Rdata` (là où s'exécute <i class="fab fa-r-project"></i>).
]
...
#  Comment Réinitialiser <i class="fab fa-r-project"></i>&nbsp;? Avec `rm(list = ls())`&nbsp;?
...
.footnote.font150[
&ast; Valeur par défaut <i class="fab fa-r-project"></i> v4.0.0 ([Kurt Hornik: stringsAsFactors](https://developer.r-project.org/Blog/public/2020/02/16/stringsasfactors/)).
]
...
#  `setwd(...)` en première ligne d'un script ...
